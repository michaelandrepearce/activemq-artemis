...
public class SecureConfigurationTest extends ActiveMQTestBase {
   ...
   public void testSecureSharedDurableSubscriber() throws Exception {
      ...
      try {
         server.start();
         internal_testSecureSharedDurableSubscriber(getConnectionFactory());
      } finally {
         try {
            server.stop();
         } catch (Exception e) {
   ...
   private void internal_testSecureSharedDurableSubscriber(ConnectionFactory connectionFactory) throws JMSException {
      ...
      try {
         ...
         Assert.fail("Security exception expected, but did not occur, excpetion expected as not permissioned to dynamically create queue");
   ...
   public void testSecureSharedSubscriber() throws Exception {
      ...
      try {
         server.start();
         internal_testSecureSharedSubscriber(getConnectionFactory());
      } finally {
         try {
            server.stop();
         } catch (Exception e) {
   ...
   public void testSecureDurableSubscriber() throws Exception {
      ...
      try {
         server.start();
         internal_testSecureDurableSubscriber(getConnectionFactory());
      } finally {
         try {
            server.stop();
         } catch (Exception e) {
   ...
   private ConnectionFactory getConnectionFactory() {
      switch (protocol) {
         case "CORE": return getActiveMQConnectionFactory();
         case "AMQP" : return getAMQPConnectionFactory();
         case "OPENWIRE": return getOpenWireConnectionFactory();
   ...
   private ActiveMQConnectionFactory getActiveMQConnectionFactory() {
      ...
      activeMQConnection.setUser("b");
      activeMQConnection.setPassword("b");
      return activeMQConnection;
   }

   private JmsConnectionFactory getAMQPConnectionFactory() {
      JmsConnectionFactory jmsConnectionFactory = new JmsConnectionFactory("amqp://localhost:61616");
      jmsConnectionFactory.setUsername("b");
      jmsConnectionFactory.setPassword("b");
   ...
   private org.apache.activemq.ActiveMQConnectionFactory getOpenWireConnectionFactory() {
      ...
      activeMQConnectionFactory.setUserName("b");
      activeMQConnectionFactory.setPassword("b");
