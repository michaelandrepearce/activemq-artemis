...
public class ShardedMessageGroups implements MessageGroups {
   ...
   public Consumer consumer(List<Consumer> consumers, MessageReference messageReference) {
      SimpleString groupId = MessageGroups.extractGroupId(messageReference);
      if (groupId == null) {
         return null;
      } else {
         int shard = groupId.hashCode() % consumers.size();
         return consumers.get(shard);
      }
   }
